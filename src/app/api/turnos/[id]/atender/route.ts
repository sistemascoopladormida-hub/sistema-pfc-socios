...
async function handleAtender(_: Request, { params }: Params) {
  try {
    ...
    if (turnoDateTime.getTime() > Date.now()) {
      return NextResponse.json({
        success: false,
        error: "Solo se puede marcar como ATENDIDO cuando el horario ya pasó",
      });
    }
    ...
    const result = await transitionTurnoEstado(pool, {
      ...
      observacionesHistorial: "Consulta realizada",
      rules: {
        allowedEstados: ["RESERVADO"],
        invalidMessage: "Solo se pueden atender turnos reservados",
      },
    });
...
export async function PATCH(request: Request, context: Params) {
  return handleAtender(request, context);
}
