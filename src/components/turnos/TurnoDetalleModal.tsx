...
export function TurnoDetalleModal({ isOpen, turnoId, onClose, onSaved }: TurnoDetalleModalProps) {
    ...
    async function bootstrap() {
      try {
        ...
        const [turnoRes, espRes] = await Promise.all([
          fetch(`/api/turnos/${turnoId}`, { cache: "no-store", signal: controller.signal }),
          fetch("/api/especialidades", { cache: "no-store", signal: controller.signal }),
        ]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => (!open && !saving ? onClose() : undefined)}>
      <DialogContent className="max-h-[90vh] overflow-auto sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>Detalle del turno {turno ? `#${turno.id}` : ""}</DialogTitle>
          <DialogDescription>Visualiza y actualiza la información del turno.</DialogDescription>
        </DialogHeader>

        {loading || !turno ? (
          <Loading label="Cargando detalle del turno..." />
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="grid gap-1 text-sm">
              ...
              <select
                ...
                value={form.estado}
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, estado: event.target.value as TurnoEstado }))
                }
              >
